...
export interface CafeSettings { id?:string; restaurantName:string; tagline:string; phone:string; email:string; address:string; logoUrl:string; heroImageUrl:string; storyImageUrl:string; restaurantOpen:boolean; openingHours:Record<string,string>; deliveryCharge:number; taxRate:number; instagram:string; facebook:string; twitter:string; }
...
export const settingsApi={get:()=>apiCall<CafeSettings>('/settings'),update:(data:Partial<CafeSettings>)=>apiCall<CafeSettings>('/settings',{method:'PUT',body:JSON.stringify(data)})};
