...
function menuOut(m){const o=m.toObject();o.id=m._id.toString();if(o.category_id&&typeof o.category_id==='object')o.category_id=o.category_id._id?.toString()||o.category_id.toString();else o.category_id=o.category_id?.toString()||null;delete o._id;return o}
...
app.get('/api/categories',async(req,res)=>ok(res,(await Category.find().sort({sort_order:1,name:1})).map(publicCategory),'Categories loaded'));
app.post('/api/categories',auth,admin,async(req,res)=>{try{const c=await Category.create({name:req.body.name,slug:req.body.slug||req.body.name.toLowerCase().trim().replace(/[^a-z0-9]+/g,'-'),sort_order:req.body.sortOrder||0});ok(res,publicCategory(c),'Category created')}catch(e){fail(res,e.message)}});
app.put('/api/categories/:id',auth,admin,async(req,res)=>{const c=await Category.findByIdAndUpdate(req.params.id,{$set:{...(req.body.name&&{name:req.body.name}),...(req.body.slug&&{slug:req.body.slug}),...(req.body.sortOrder!==undefined&&{sort_order:req.body.sortOrder})}},{new:true});c?ok(res,publicCategory(c),'Category updated'):fail(res,'Category not found',404)});
app.delete('/api/categories/:id',auth,admin,async(req,res)=>{await Category.findByIdAndDelete(req.params.id);await Menu.updateMany({category_id:req.params.id},{$set:{category_id:null}});ok(res,null,'Category deleted')});

app.get('/api/menu',async(req,res)=>ok(res,(await Menu.find().sort({sort_order:1,name:1})).map(menuOut),'Menu loaded'));
...
async function seedAdmin(){const email=(process.env.ADMIN_EMAIL||'').trim().toLowerCase();const password=process.env.ADMIN_PASSWORD||'';if(!email&&!password){console.log('Admin seed skipped: ADMIN_EMAIL and ADMIN_PASSWORD are not set.');return}if(!email||!password)throw new Error('ADMIN_EMAIL and ADMIN_PASSWORD must both be set for admin seeding');if(password.length<8)throw new Error('ADMIN_PASSWORD must be at least 8 characters');const existing=await User.findOne({email});const passwordHash=await bcrypt.hash(password,12);if(existing){if(existing.role!=='admin')existing.role='admin';existing.passwordHash=passwordHash;await existing.save();console.log(`Admin account ready: ${email}`);return}await User.create({email,passwordHash,firstName:'Admin',role:'admin'});console.log(`Admin account created: ${email}`)}
